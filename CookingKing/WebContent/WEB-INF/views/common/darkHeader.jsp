...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<c:set var="memberLoggedIn" value="${ sessionScope.memberLoggedIn }"></c:set>
<c:set var="adminGrade" value="<%= UserService.USER_ADMIN %>"/>
...
<meta charset="UTF-8">
<title>스케쥴</title>
<link rel="stylesheet" href="<c:url value="/css/dark-header.css"></c:url>" />
 <script src="<c:url value="/js/jquery-3.5.1.js"></c:url>"></script>
    <script>
        $(function(){
...
/*         function lostPw(){
        ...
        $(function(){
        	$("#logout").click(function(){
            	...
            	if(result){
            		location.href = '<c:url value="/user/logout"></c:url>';
        ...
        //비밀번호 재설정
        function lostPw(){
        	location.href = "<c:url value='/user/checkInfo'></c:url>";
        	
        };
        
        //리뷰팝업
	        function writeReview(){
        	var url = "<c:url value='/user/writeReview'></c:url>";
        ...
        function msg(){
        	var url = "<c:url value='/user/message?userId=${ memberLoggedIn != null ? memberLoggedIn.userId : ""}'></c:url>";
    ...
    <div class="wrapper-user-header">
        <div class="user-header">
            <div class="user-navbar">
                ...
                <a href="${ pageContext.request.contextPath }"><button id="logo-btn" class="logo-btn">cooking-king</button></a>
            <button class="user-navbar1" onclick="location.href='<c:url value="/board/list"></c:url>'">클래스</button>
            <button class="user-navbar2" onclick="location.href='<c:url value="/member/schedule"></c:url>'">내 스케줄</button>
       ...
       <div class="login-wrapper">
            <div class="mypage-login" id="mypage-login">
                ...
                action="<c:url value='/user/login'></c:url>"
					  method="POST">
                    ...
                    <input type="password" class="input-login-password" id="input-login-password" name="password"
                        ...
                        <br />
                        <a href="<c:url value='user/enroll'></c:url>" 
...
<c:if test="${ memberLoggedIn != null && memberLoggedIn.commGrade ne adminGrade }">
      ...
      <div class="wrapper-user-header">
        <div class="user-header">
            <div class="user-navbar">
                ...
                <a href="${ pageContext.request.contextPath }"><button id="logo-btn" class="logo-btn">cooking-king</button></a>
            <button class="user-navbar1" onclick="location.href='<c:url value="/board/list"></c:url>'">클래스</button>
            <button class="user-navbar2" onclick="location.href='<c:url value="/member/schedule?userId=${ memberLoggedIn.userId }"></c:url>'">내 스케줄</button>
            <button class="user-navbar4" id="mypage-btn" onclick="mypage();">마이페이지</button>
            <button class="user-navbar5" onclick="location.href='<c:url value="/center/list"></c:url>'">고객센터</button>
                    </div>
            </div>
      <div class="mypage-loggedIn" id="mypage-loggedIn">
   		<div class="hello" id="hello">
   			<p class="hello-p">${ memberLoggedIn.userName }님, 안녕하세요.</p>
   		</div>
        <div class="mypage" id="mypage">
            ...
            <button class="mypages" id="write-resume" onclick="location.href='<c:url value="/tutor/writerResume?userId=${ memberLoggedIn.userId }"></c:url>'">이력서 관리</button><br />
 ...
 <c:if test="${ memberLoggedIn != null &&  memberLoggedIn.commGrade eq adminGrade }">
     	    <div class="wrapper-admin-header">
       			 <div class="admin-header">
          			  <div class="admin-navbar">
                    <div class="navbar-admin-menu">
                <a href="${ pageContext.request.contextPath }"><button id="logo-btn-a">cooking-king</button></a>
            <button class="admin-navbar0" id="admin-logout" onclick="adminLogout();">로그아웃</button>
            <button class="admin-navbar1" onclick="location.href='<c:url value="/board/list"></c:url>'">클래스 관리</button>
            <button class="admin-navbar2" onclick="location.href='<c:url value="/admin/page"></c:url>'">회원 관리</button>
            <button class="admin-navbar4" onclick="location.href='<c:url value="/admin/analysis"></c:url>'">현황 조회</button>
            <button class="admin-navbar5" onclick="location.href='<c:url value="/center/list"></c:url>'">고객센터</button>
                    </div>
            </div>
       			 <div class="helloAdmin" id="helloAdmin">
   					<p class="helloAdmin-p" onclick="adminLogout();">
   					${ memberLoggedIn.userName }님, 안녕하세요. </p>
   				 </div>
        </div>
    </div>
