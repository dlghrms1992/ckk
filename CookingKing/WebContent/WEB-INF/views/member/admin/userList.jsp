...
%>
	...
	<tr>
		...
		<td><%= u.getPhone() %></td>
		<td><%= u.getEmail() %></td>
		<td><input type="button" value="X" id="piss-off"onclick="location.href = '<%= request.getContextPath() %>/user/delete?userId=<%= u.getUserId() %>'" /></td>
		<td><%= pointF.format(u.getPointSum()) %></td>
		<td><input type="button" value="포인트관리" id="p-mng" onclick="location.href = '<%= request.getContextPath() %>/admin/viewPointLog?userId=<%= u.getUserId() %>'" /></td>
	</tr>
